...
client = TherminalClient()
obs = client.observations(station="LAX", resolution="1min", limit=2, as_dataframe=True)
```
...
<ApiPlayground
  ...
  params={[
    ...
    { name: "resolution", placeholder: "hourly" },
...
Live observations from [AWC](https://aviationweather.gov) (5-second polling). Historical from [IEM](https://mesonet.agron.iastate.edu) and [NOAA ISD](https://www.ncei.noaa.gov/products/land-based-station/integrated-surface-database). 1-minute ASOS data from [NCEI](https://www.ncei.noaa.gov) (DSI-6406). See [Data Sources](/data-sources/) for full details.
