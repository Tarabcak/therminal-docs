...
The [Python SDK](/sdk/) (v1.0.0) provides domain-split clients, a config file, and **LiveClient** for real-time METAR observations from AWC with schema-identical output to historical data.

```python
from therminal.weather import WeatherClient, LiveClient

weather = WeatherClient()                              # Historical (via API)
live = LiveClient()                                    # Real-time (via AWC)
...
- [Python SDK](/sdk/) — Domain clients, LiveClient, config, ML features
