...
Requires Python 3.11+. Current version: **v1.0.9**. Published on [PyPI](https://pypi.org/project/therminal-py/).

## What's new in v1.0.9

- **Fix: parquet null-to-zero**: Nullable observation fields (wind_gust_kt, feels_like_f, sky layers, etc.) now correctly produce `NaN` in parquet DataFrames instead of being silently coerced to `0`. The API's parquet schema now uses `Optional` for nullable columns.
- 7 new regression tests for parquet null preservation
